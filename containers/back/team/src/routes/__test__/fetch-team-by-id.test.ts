...
it("returns 200 after fetching a team by id", async () => {
...
it("returns 401 after fetching a team by id if unauthorized", async () => {
  ...
  await request(app).get(`/api/v1/teams/${teams[0]._id}`).expect(401);
});

it("returns 400 after fetching a team by wrong id", async () => {
  await createProfile(userId, userName, profileHandle);
  ...
  await request(app)
    .get(`/api/v1/teams/${mongoose.Types.ObjectId().toHexString()}`)
    .set("Authorization", token)
    .expect(400);
});
