...
const deleteUserFromTeam = async (req: UserRequest, res: Response) => {
  ...
  if (!profile) {
    ...
    if (team.members.filter((member) => member.toString() === profileToDelete.id).length === 0) {
      throw new BadRequestError('User is not a member of this team');
    }

    // Get the remove index
    const removeIndex = team.members.map((item) => item._id.toString()).indexOf(profileToDelete.id);
