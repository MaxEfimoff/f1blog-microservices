...
const secondGroupTitle = 'fakegro7686';
...
it('returns 200 after fetching all groups in team', async () => {
  ...
  const firstGroup = res4.body.data.groups[1];
  const secondGroup = res4.body.data.groups[0];

  expect(firstGroup.title).toEqual(groupTitle);
