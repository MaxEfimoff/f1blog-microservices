...
const dislikeNewsItemThread = async (req: UserRequest, res: Response) => {
  const user = req.user;
  ...
  if (!profile) {
    ...
    if (
      ...
      throw new BadRequestError('You already disliked this thread');
    }

    newsItemthread.dislikes.unshift(profile);
